...
What you can see in the main Kwirth is:
  ...
  - The toolbar, with your login info (the user on the right), the name of the current workspace (just before the user), we will see later what is a workspace. On the very left you'll find the access to the main menu, the burger icon.
  ...
  - The resource selector. This is one of the most important things in Kwirth.
    ...
    The resource selector is the tool you use to access logs on your source Kubernetes cluster. It has **6 selectors** and the 'add' button (to add the surce object to the workspace):
  ...
  - The main menu gives you access to several configuration and operation options of your Kwirth installation.
    ...
    - Next you have options for exporting your workspaces (a set of logs) to a file or import them from a file.
...
### The *workspaces*
A workspace is a set of log objects, where a log object can have different forms depending on the log configuration:
...
Where the content of the workspace is as follows:
  ...
  - Log objects. When you add a source object from the resource selector (by clicking the 'add' button), a new tab would be added to the workspace, with a default name automatically created depending on the scope and the source object. Tabs can be renamed once created, but their names must be unique.
  - Filter. You can filter what info is shown on the log object (a tab in the workspace). Filters are the basis for the alarms.
...
To create an alarm...:
  ...
  - Configure alarm settings:
    ...
    - Behaviour
    - Sound
  - When yo click 'Save' the alarm is stored in your workspace and starts running.
