...
+++Starting with Kwirth 0.3.160 the core has been split into 2 separete blocks:
  ...
  2. The interface for implementing features in Kwirth, that is what we call **back channels**
...
According to this structure the different features that users expect to use with Kwirth are what we call **channels**. For example, receive real-time log streams is just one feature, so there exists a **channel** that implements this feature in the backend an dalso in the forntend.
...
We have plans to integrate channels into **'plugins'**, exactly the way ![Backstage](https://backstage.io/plugins) plugins works.
