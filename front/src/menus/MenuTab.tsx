...
const MenuTab: React.FC<any> = (props:IProps) => {
    ...
    const menuTabs=(
        <Menu id='menu-logs' anchorEl={props.anchorMenuTab} open={Boolean(props.anchorMenuTab)} onClose={props.onClose}>
            ...
            <MenuItem key='fa' onClick={() => props.optionSelected(MenuTabOption.AlarmCreate)} sx={{ml:3}}>Convert filter to alarm...</MenuItem>
            <MenuItem key='ma' onClick={() => props.optionSelected(MenuTabOption.ManageAlarms)} sx={{ml:3}}>Manage alarms...</MenuItem>
            <Divider/>
            ...
            <Collapse in={subMenuTabOpen} timeout="auto" unmountOnExit sx={{ml:5}}>
                <MenuItem key='tabil' onClick={() => props.optionSelected(MenuTabOption.TabInfo)} disabled={props.selectedTabIndex<0}><Info/>&nbsp;Log information</MenuItem>
                <MenuItem key='tabrl' onClick={() => props.optionSelected(MenuTabOption.TabRename)} disabled={props.selectedTabIndex<0}><DriveFileRenameOutline/>&nbsp;Rename log</MenuItem>
                <MenuItem key='tabdl' onClick={() => props.optionSelected(MenuTabOption.TabSetDefault)} disabled={props.selectedTabIndex<0} sx={{ml: props.selectedTab?.defaultTab?0:3}}> {props.selectedTab?.defaultTab && <Check/>} Default log</MenuItem>
            ...
            <MenuItem key='sublog' onClick={submenuLogClick} sx={{ml:3}}>Actions<Typography sx={{flexGrow:1}}></Typography>{subMenuLogOpen ? <ExpandLess/> : <ExpandMore/>}</MenuItem>
            <Collapse in={subMenuLogOpen} timeout="auto" unmountOnExit sx={{ml:5}}>
                ...
                <MenuItem key='logremove' onClick={() => props.optionSelected(MenuTabOption.TabRemove)} ><RemoveCircleRounded/>&nbsp;Remove</MenuItem>
            </Collapse>
            ...
            <Collapse in={submenuMetricsOpen} timeout="auto" unmountOnExit sx={{ml:5}}>
                ...
                <MenuItem key='metricsremove' onClick={() => props.optionSelected(MenuTabOption.MetricsRemove)}><RemoveCircleRounded/>&nbsp;Remove</MenuItem>
            </Collapse>
