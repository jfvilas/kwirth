...
const NotificationMenu: React.FC<NotificationMenuProps> = ({ anchorEl, open, onClose, onClear, messages, onDelete, channels }) => {
	...
	return (
		...
		>
			...
			<List sx={{ p: 0 }}>
				{messages.length > 0 ? (
					messages.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime()).map((msg, index) => (
						<ListItem 
							key={index}
							divider
							secondaryAction={
